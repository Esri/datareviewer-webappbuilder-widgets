  ...
  return declare([BaseWidget, _WidgetsInTemplateMixin], {
    ...
    _createRadioButtons:function(radioArray){
      ...
      for(var i = 0; i < radioArray.length; i++){
        ...
        "Link'><input type='radio' name='fieldNamesRadio' value='" +
        radioArray[i].fieldName + "'";
    ...
    _radioChange:function(event){
      ...
      if(event.target.checked && this.useCurrentMapExtent){
        mapSelectionArea = this._getMapExtent();
      }
      else if (event.target.checked && this.useCurrentMapExtent === false){
    ...
    */
    ...
    _createCharts:function(dashboardResult, layerDrawingOptions, fieldName){
      ...
      var revLifecycle = new ReviewerLifecycle();
      ...
      array.forEach(resultOtherCategory.fieldValues, lang.hitch(this, function(item) {
        ...
        item !== this.nls.OtherCategory){
          chartDisplayText = revLifecycle.toLifecycleStatusString(item);
        }
        else if (resultOtherCategory.fieldName.toLowerCase() ===
        this.nls.phaseFieldName.toLowerCase()){
          chartDisplayText = revLifecycle.toLifecyclePhaseString(parseInt(item, 10));
        }
        else{
        ...
        series.push({
          ...
          chartDisplayText + ":</span><br/><span> " +
          resultOtherCategory.getCount(item) + "</span></div>",
          color: chartColor
        });
      }));
      var pieChartMedia = {
    ...
    _creatBarsChart: function(chartDiv, series) {
      ...
      barsChart.connectToPlot("default", lang.hitch(this, function(args) {
        if(args.type === "onclick") {
          if(args.run.chart.series[0].data[args.index].name === "Other"){
            this.chartPageLevel += 1;
    ...
    */
    getLifecyclePhaseresult:function (result){
      ...
      var indexVerified = -1;
      var reviewerLifecycle = new ReviewerLifecycle();
